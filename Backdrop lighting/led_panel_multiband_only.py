"""
led_panel_multiband_only.py
----------------------------
Just the MULTI-BAND MAPPED pattern: each of the 6 panels is permanently
tied to its own frequency range (Low/Mid-Vocal/High, mirrored order:
Low-Mid-High-High-Mid-Low). Brightness of each panel tracks ONLY its own
range's amplitude, so multiple panels can brighten together for mixed
content.
...
USAGE:
    python led_panel_multiband_only.py --file song.wav
    python led_panel_multiband_only.py --demo
    python led_panel_multiband_only.py --mic
"""
...
def main():
    ...
    fig.suptitle("Multi-band mapped - each panel tied to its own frequency range", color="white")
