"""
led_panel_combined_vu_only.py
------------------------------
Just the COMBINED (VU METER) pattern: panels light up one by one,
left to right, based on how loud the audio currently is - more panels
lit means louder, like a classic VU meter.
...
USAGE:
    python led_panel_combined_vu_only.py --file song.wav
    python led_panel_combined_vu_only.py --demo
    python led_panel_combined_vu_only.py --mic
"""
...
def layout(band_energies, overall, frame):
    # symmetric VU: light mirrored pairs together, growing outward from the
    # center as it gets louder, instead of a plain left-to-right sweep -
    # pairs are (2,3) center, (1,4) middle, (0,5) outer
    pairs = [(2, 3), (1, 4), (0, 5)]
    steps_lit = int(round(overall * len(pairs)))  # how many pairs are lit, 0-3
    vals = np.full(PANEL_COUNT, BASELINE)
    for i, (a, b) in enumerate(pairs):
        if i < steps_lit:
            vals[a] = vals[b] = BASELINE + (1 - BASELINE) * overall
    return vals
...
def main():
    ...
    fig.suptitle("Combined (VU) - panels fill left-to-right with loudness", color="white")
