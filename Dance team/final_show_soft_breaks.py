#!/usr/bin/env python3
"""
Final show — SOFT BREAKS variant.

Same music sequencing as the hard-blackout version, but the 6 SHORT
dialogue points (under ~15s) go to a low, slow "Ember" glow instead of
true black -- present but clearly receded, so the stage isn't flipping
to full black six separate times across the piece. The 3 LONG dialogue
points (the opener, the mid-point, and the closer) still go to a true
hard blackout, since those are long enough to read as a deliberate beat.
...
RUN (from the folder containing this script, stage_photo.png, and the track):
    python final_show_soft_breaks.py --file "Last_performance_track_with_voicee_mp3.mpeg"
...
# ---------------------------------------------------------------
TIMELINE = [
    (0.00,   37.00,  "Dialogue - BLACKOUT (opener)", seq_blackout),
    (37.00,  201.00, "Depth-Pair Sweep",            seq_depth_sweep),   # 0:37-3:21
    (201.00, 215.00, "Dialogue - Ember",            seq_ember),         # 3:21-3:35
    (215.00, 313.00, "Sparkle Flicker",             seq_sparkle),       # 3:35-5:13
    (313.00, 327.00, "Dialogue - Ember",            seq_ember),         # 5:13-5:27
    (327.00, 478.00, "Chase",                       seq_chase),         # 5:27-7:58
    (478.00, 487.00, "Dialogue - Ember",            seq_ember),         # 7:58-8:07
    (487.00, 606.00, "Multiband Pairs",             seq_multiband),     # 8:07-10:06
    (606.00, 617.00, "Dialogue - Ember",            seq_ember),         # 10:06-10:17
    (617.00, 724.00, "Left <-> Right Alternate",    seq_lr_alternate),  # 10:17-12:04
    (724.00, 744.00, "Dialogue - BLACKOUT (mid)",   seq_blackout),      # 12:04-12:24
    (744.00, 977.00, "All-Together Pulse",          seq_all_pulse),     # 12:24-16:17
    (977.00, 984.00, "Dialogue - Ember",            seq_ember),         # 16:17-16:24
    (984.00, 1094.00,"Depth-Pair Sweep",            seq_depth_sweep),   # 16:24-18:14
    (1094.00,1104.00,"Dialogue - Ember",            seq_ember),         # 18:14-18:24
    (1104.00,1264.00,"Chase",                       seq_chase),         # 18:24-21:04
    (1264.00,1287.00,"Dialogue - BLACKOUT (closer)",seq_blackout),      # 21:04-21:27
]
